...
def process_image(input_path: str, output_path: str, key: int):
    """
    Encrypts or decrypts an image by performing a bitwise XOR operation
    with a key on each pixel value.
    
    :param input_path: Path to the input image file.
    :param output_path: Path where the output image should be saved.
    :param key: An integer key (0-255) used as the XOR operand.
    """
    ...
    try:
        # Normalize the key to fit within a single byte (0-255)
        byte_key = key % 256
        ...
        with Image.open(input_path) as img:
            ...
            # Use RGBA if alpha exists, else RGB
            ...
            # If the original was JPEG, save as the same or similar
            # Note: Lossy compression like JPEG will slightly modify pixel values during save,
            # which can corrupt XOR decryption! Therefore, PNG is highly recommended for lossless 
            # storage of encrypted image data. We warn the user about this.
...
def main():
    ...
    while True:
        try:
            ...
            # Clean quotes if user dragged and dropped the file
